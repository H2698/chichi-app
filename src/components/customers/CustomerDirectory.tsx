...
export function CustomerDirectory({ admin = false }: { admin?: boolean }) {
  ...
  return (
    <div className={`px-[22px] pb-10 pt-2.5 ${admin ? "lg:px-0 lg:pt-0" : ""}`}>
      ...
      /> : (
        <div className="mb-6">
          <BackHeader title="Clientes" onBack={() => router.push("/home")} />
          <div className="mt-2 text-[13px] text-secondary-2">
      ...
      <div className="flex items-center gap-[11px] rounded-2xl border border-border-input bg-card px-[15px] py-3.5">
        ...
        <input
          type="search"
          aria-label="Rechercher une cliente par nom ou téléphone"
          value={query}
      ...
      {results.length === 0 ? (
        ...
        <div className={`mt-5 grid gap-2.5 ${admin ? "lg:grid-cols-2" : ""}`}>
          {results.map((c) => (
            <Link
              key={c.id}
              href={`${admin ? "/admin/customers" : "/customers"}/${encodeURIComponent(c.id)}`}
