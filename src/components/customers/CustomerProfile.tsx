...
export function CustomerProfile({ admin = false }: { admin?: boolean }) {
  ...
  if (!customer) {
    return (
      <div className="px-[22px] pb-10 pt-2.5 text-center text-secondary-2 lg:px-0 lg:pt-0">
        Cliente introuvable.
        <Link className="mt-4 block" href={admin ? "/admin/customers" : "/customers"}>
          Retour aux clientes
        </Link>
      </div>
    );
    ...
    return (
      <Link
        href={admin
          ...
          : `/reservations?highlight=${encodeURIComponent(r.id)}&day=${r.pickupDay}`}
        className="flex cursor-pointer flex-wrap items-center justify-between gap-3 rounded-2xl border border-border bg-card px-4 py-3"
      >
        <div>
  ...
  return (
    <div className={`mx-auto max-w-[640px] px-[22px] pb-10 pt-2.5 ${admin ? "lg:mx-0 lg:px-0 lg:pt-0" : ""}`}>
      {admin ? (
        <AdminPageHeader title={`${customer.firstName} ${customer.lastName}`} back subtitle={customer.phone} />
      ) : (
        <div className="mb-6">
          <BackHeader title={`${customer.firstName} ${customer.lastName}`} onBack={() => router.push("/customers")} />
          <div className="mt-3 text-[14px] text-secondary-2">{customer.phone}</div>
          {customer.email && <div className="mt-1 break-words text-[13px] text-secondary-2">{customer.email}</div>}
        </div>
      )}
      ...
      <div className="mt-6">
        ...
        <div className="mt-3 flex flex-col gap-2">
          {past.length === 0 ? (
            <div className="rounded-2xl border border-border bg-card px-4 py-4 text-center text-[13px] text-secondary-2">
              Aucune location passée enregistrée.
            </div>
          ) : (
